...
describe("Infrastructure testing", () => {
  ...
  test("getVocabularyFile", async () => {
    ...
    debugFunc(mySA.retrievalMemory);
  ...
  test("vocabularyFileStructure", async () => {
    ...
    expect(util.isObject(mySA.retrievalMemory.versionsFile.releaseLog)).toBe(
      true
    );
    expect(util.isString(mySA.retrievalMemory.versionsFile.schemaversion)).toBe(
      true
    );
  ...
  test("latestVersionIsCorrect", async () => {
    ...
    const sortedVersionsArray = util.sortReleaseEntriesByDate(
      mySA.retrievalMemory.versionsFile.releaseLog
    );
    ...
    debugFunc(
      "Latest version by schema-org-adapter: " + mySA.retrievalMemory.latest
    );
    expect(sortedVersionsArray[0][0] === mySA.retrievalMemory.latest).toBe(
      true
    );
  ...
  test("getAllVocabularyVersions", async () => {
    ...
    await mySA.getSDOVersionFile();
    for (const currentVersion of Object.keys(
      mySA.retrievalMemory.versionsFile.releaseLog
    )) {
      let currentFileURL;
      try {
