...
const VOC_OBJ_SDO_3_7 = require("./data/schema-3.7.json");
const VOC_OBJ_SDO_10 = require("./data/schema-10.0.json");
const VOC_OBJ_ZOO_2 = require("./data/vocabulary-animal-2.json");
...
describe("utilities testing", () => {
  ...
  test("toCompactIRI()", async () => {
    expect(util.toCompactIRI("http://schema.org/Book", testContext)).toBe(
      "schema:Book"
    );
    expect(
      util.toCompactIRI("http://schema.org/Book", testContext, false)
    ).toBe("schema:Book");
    expect(
      util.toCompactIRI("https://schema.org/Book", testContext, true)
    ).toBe("schema:Book");
    expect(
      util.toCompactIRI("https://schema.org/Book", testContext, false)
    ).toBe(null);
  });
  ...
  test("discoverEquateNamespaces()", async () => {
    expect(
      Array.isArray(util.discoverEquateNamespaces(testContext, VOC_OBJ_ZOO))
    ).toBe(true);
    expect(util.discoverEquateNamespaces(testContext, VOC_OBJ_ZOO).length).toBe(
      1
    );
    expect(util.discoverEquateNamespaces(testContext, VOC_OBJ_ZOO)[0]).toBe(
      "https://schema.org/"
    );
    expect(
      util.discoverEquateNamespaces(testContext, VOC_OBJ_ZOO_2).length
    ).toBe(1);
    expect(util.discoverEquateNamespaces(testContext, VOC_OBJ_ZOO_2)[0]).toBe(
      "https://schema.org/"
    );
    expect(
      util.discoverEquateNamespaces(VOC_OBJ_ZOO["@context"], VOC_OBJ_SDO_3_7)[0]
  ...
  test("getFileNameForSchemaOrgVersion()", async () => {
    ...
    for (const currVersion of Object.entries(expectedFileMapping)) {
      ...
      if (currVersion[1] === null) {
        ...
        expect(() => {
          util.getFileNameForSchemaOrgVersion(currVersion[0]);
        }).toThrow();
      } else {
        // eslint-disable-next-line jest/no-conditional-expect
        expect(util.getFileNameForSchemaOrgVersion(currVersion[0])).toBe(
          currVersion[1]
        );
      }
    }
