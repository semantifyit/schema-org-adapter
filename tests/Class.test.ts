...
describe("Class methods", () => {
  ...
  test("getProperties()", async () => {
    ...
    expect(crWork.getProperties(true)).not.toContain(
  ...
  test("isRangeOf()", async () => {
    ...
    expect(thing.isRangeOf().length === thing.isRangeOf(false).length).toBe(
      true
    );
    ...
    expect(
      restaurant.isRangeOf().length === foodEstablishment.isRangeOf().length
    ).toBe(true);
  });
