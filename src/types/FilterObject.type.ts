 ...
 * }
 *
 * // the following filter can be passed to a function to retrieve only terms that are superseded
 * {
 *   isSuperseded: true
 * }
 * ```
...
export type FilterObject = {
  ...
  /**
   * Namespaces for vocabularies (e.g. `"https://schema.org/"`) can be passed here, which matches only vocabulary terms that use any of the given namespaces in their IRI. You can check the namespaces and corresponding identifiers (e.g. `"schema"`) used by an {@link SDOAdapter | SDO-Adapter} instance with {@link SDOAdapter.getVocabularies | .getVocabularies()}. It is also possible to get the corresponding namespace of Term instance with {@link Class.getVocabulary | .getVocabulary()}.
   */
